...
// create a new meetup and verify the step is 1 of 4 and the step title; then go to the next step
        ...
        delay.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(CreateMeetupSelectors.step1)));
        Assert.assertTrue(driver.findElement(By.xpath(CreateMeetupSelectors.step1)).isDisplayed());
...
// verify the step is 2 of 4 and the step title; select the new technology preference then go to the next step
        delay.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(CreateMeetupSelectors.step2)));
        Assert.assertTrue(driver.findElement(By.xpath(CreateMeetupSelectors.step2)).isDisplayed());
...
// verify step is 3 of 4 nad the step title; enter the meetup title and description and go to the next step
        delay.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(CreateMeetupSelectors.step3)));
        Assert.assertTrue(driver.findElement(By.xpath(CreateMeetupSelectors.step3)).isDisplayed());
...
// verify step is 4 of 4 and the help section; click agree and continue
        delay.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(CreateMeetupSelectors.step4)));
        Assert.assertTrue(driver.findElement(By.xpath(CreateMeetupSelectors.step4)).isDisplayed());
